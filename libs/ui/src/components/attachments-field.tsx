...
/**
 * Manage a row's attachments. In **edit** mode (`ownerId` set) each add / remove
 * hits the server immediately; when **creating** (`ownerId` null) the files are
 * uploaded to Blob and staged in the form, then attached once the owner row
 * exists. `upload` / `remove` / `discard` are supplied by the feature.
 */
export function AttachmentsField({
  ...
  onSavedChange: (next: StoredAttachment[]) => void;
  ...
  /** Override when a page renders more than one field. */
  inputId?: string;
}) {
  ...
  const items: Array<StoredAttachment | AttachmentDraft> = ownerId
    ? saved
    : drafts;
  ...
  const remove = async (a: StoredAttachment | AttachmentDraft) => {
    if ('id' in a) {
