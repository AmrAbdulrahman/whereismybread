/**
 * Shared attachment rules — imported by the client picker/viewer, the upload
 * route handler, and feature server actions. No secrets, no I/O, no zod (each
 * feature keeps its own `attachmentDraftSchema` — this lib has no zod dep).
 */
 ...
 * (scoped to the owning user — see the route handler).
 */
...
/** A file stored in Blob and recorded against a saved row. */
...
/** A file uploaded to Blob but not yet attached to a saved row. */
